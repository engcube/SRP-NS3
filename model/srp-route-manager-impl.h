...
#ifndef SRP_ROUTE_MANAGER_IMPL_H
#define SRP_ROUTE_MANAGER_IMPL_H
 ...
 *
 * Each SPFVertex has a pointer to the SRP Router Link State Advertisement
 * (LSA) that its underlying router has exported.  Within these LSAs are
 * SRP Router Link Records that describe the point to point links from the
...
/**
 * @brief Get the SRP Router Link State Advertisement returned by the 
 * SRP Router represented by this SPFVertex during the route discovery 
 * process.
 * @internal
 *
 * @see SRPRouter
 * @see SRPRoutingLSA
 * @see SRPRouter::DiscoverLSAs ()
 * @returns A pointer to the SRPRoutingLSA found by the router represented
 * by this SPFVertex object.
 */
  SRPRoutingLSA* GetLSA (void) const;

/**
 * @brief Set the SRP Router Link State Advertisement returned by the 
 * SRP Router represented by this SPFVertex during the route discovery 
 * process.
 * @internal
 *
 ...
 * @param lsa A pointer to the SRPRoutingLSA.
 */
  void SetLSA (SRPRoutingLSA* lsa);
 ...
 * packets is determined by examining the SRP Router Link Records of the
 * Link State Advertisement generated by the root node's SRPRouter.  This
 ...
 * described by one of these SRP Router Link Records.
 * 
...
/**
 * @brief The Link State DataBase (LSDB) of the SRP Route Manager.
 *
 * Each node in the simulation participating in SRP routing has a
 * SRPRouter interface.  The primary job of this interface is to export
 * SRP Router Link State Advertisements (LSAs).  These advertisements in
 * turn contain a number of SRP Router Link Records that describe the 
 ...
 */
class SRPRouteManagerLSDB
{
public:
/**
 * @brief Construct an empty SRP Router Manager Link State Database.
 * @internal
 *
...
/**
 * @brief Destroy an empty SRP Router Manager Link State Database.
 * @internal
 *
 ...
 * @internal
 *
 * The IPV4 address and the SRPRoutingLSA given as parameters are converted
 ...
 */
  void Insert (Ipv4Address addr, SRPRoutingLSA* lsa);
 ...
 */
  SRPRoutingLSA* GetLSA (Ipv4Address addr) const;
/**
 * @brief Look up the Link State Advertisement associated with the given
 ...
 * ID.
 */
  SRPRoutingLSA* GetLSAByLinkData (Ipv4Address addr) const;
 ...
 */
  ...
  SRPRoutingLSA* GetExtLSA (uint32_t index) const;
  uint32_t GetNumExtLSAs () const;


private:
  typedef std::map<Ipv4Address, SRPRoutingLSA*> LSDBMap_t;
  typedef std::pair<Ipv4Address, SRPRoutingLSA*> LSDBPair_t;

  LSDBMap_t m_database;
  std::vector<SRPRoutingLSA*> m_extdatabase;

/**
 * @brief SRPRouteManagerLSDB copy construction is disallowed.  There's no 
 * need for it and a compiler provided shallow copy would be wrong.
 */
  SRPRouteManagerLSDB (SRPRouteManagerLSDB& lsdb);

/**
 * @brief The SPFVertex copy assignment operator is disallowed.  There's no 
 * need for it and a compiler provided shallow copy would be wrong.
 */
  SRPRouteManagerLSDB& operator= (SRPRouteManagerLSDB& lsdb);
};

/**
 * @brief A SRP router implementation.
 *
 * This singleton object can query interface each node in the system
 * for a SRPRouter interface.  For those nodes, it fetches one or
 ...
 */
class SRPRouteManagerImpl
{
public:
  SRPRouteManagerImpl ();
  virtual ~SRPRouteManagerImpl ();
/**
 * @brief Delete all static routes on all nodes that have a
 * SRPRouterInterface
 *
 * TODO:  separate manually assigned static routes from static routes that
 * the SRP routing code injects, and only delete the latter
 * @internal
 *
 */
  virtual void DeleteSRPRoutes ();

/**
 * @brief Build the routing database by gathering Link State Advertisements
 * from each node exporting a SRPRouter interface.
 * @internal
 */
  virtual void BuildSRPRoutingDatabase ();
 ...
 * @internal
 */
  void DebugUseLsdb (SRPRouteManagerLSDB*);
...
private:
/**
 * @brief SRPRouteManagerImpl copy construction is disallowed.
 * There's no  need for it and a compiler provided shallow copy would be 
 * wrong.
 */
  SRPRouteManagerImpl (SRPRouteManagerImpl& srmi);

/**
 * @brief SRP Route Manager Implementation assignment operator is
 * disallowed.  There's no  need for it and a compiler provided shallow copy
 * would be hopelessly wrong.
 */
  SRPRouteManagerImpl& operator= (SRPRouteManagerImpl& srmi);

  SPFVertex* m_spfroot;
  SRPRouteManagerLSDB* m_lsdb;
  bool CheckForStubNode (Ipv4Address root);
  void SPFCalculate (Ipv4Address root);
  void SPFProcessStubs (SPFVertex* v);
  void ProcessASExternals (SPFVertex* v, SRPRoutingLSA* extlsa);
  void SPFNext (SPFVertex*, CandidateQueue&);
  int SPFNexthopCalculation (SPFVertex* v, SPFVertex* w, 
                             SRPRoutingLinkRecord* l, uint32_t distance);
  void SPFVertexAddParent (SPFVertex* v);
  SRPRoutingLinkRecord* SPFGetNextLink (SPFVertex* v, SPFVertex* w, 
                                           SRPRoutingLinkRecord* prev_link);
  void SPFIntraAddRouter (SPFVertex* v);
  void SPFIntraAddTransit (SPFVertex* v);
  void SPFIntraAddStub (SRPRoutingLinkRecord *l, SPFVertex* v);
  void SPFAddASExternal (SRPRoutingLSA *extlsa, SPFVertex *v);
...
#endif /* SRP_ROUTE_MANAGER_IMPL_H */
