...
Ipv4Address
SRPRoutingLinkRecord::GetLinkId (void) const
{
  NS_LOG_FUNCTION (this);
  return m_linkId;
}
...
void
SRPRoutingLinkRecord::SetLinkId (Ipv4Address addr)
{
...
Ipv4Address
SRPRoutingLinkRecord::GetLinkData (void) const
{
  NS_LOG_FUNCTION (this);
  return m_linkData;
}
...
void
SRPRoutingLinkRecord::SetLinkData (Ipv4Address addr)
{
  NS_LOG_FUNCTION (this << addr);
  m_linkData = addr;
}

SRPRoutingLinkRecord::LinkType
SRPRoutingLinkRecord::GetLinkType (void) const
{
  NS_LOG_FUNCTION (this);
  return m_linkType;
}
...
void
SRPRoutingLinkRecord::SetLinkType (
  SRPRoutingLinkRecord::LinkType linkType)
{
...
uint16_t
SRPRoutingLinkRecord::GetMetric (void) const
{
  NS_LOG_FUNCTION (this);
  return m_metric;
}
...
void
SRPRoutingLinkRecord::SetMetric (uint16_t metric)
{
...
// ---------------------------------------------------------------------------
//
// SRPRoutingLSA Implementation
//
// ---------------------------------------------------------------------------

SRPRoutingLSA::SRPRoutingLSA()
  : 
    m_lsType (SRPRoutingLSA::Unknown),
    m_linkStateId ("0.0.0.0"),
    m_advertisingRtr ("0.0.0.0"),
    m_linkRecords (),
    m_networkLSANetworkMask ("0.0.0.0"),
    m_attachedRouters (),
    m_status (SRPRoutingLSA::LSA_SPF_NOT_EXPLORED),
    m_node_id (0)
{
  NS_LOG_FUNCTION (this);
}
...
SRPRoutingLSA::SRPRoutingLSA (
  SRPRoutingLSA::SPFStatus status,
...
SRPRoutingLSA::SRPRoutingLSA (SRPRoutingLSA& lsa)
    ...
    m_node_id (lsa.m_node_id)
{
  ...
  NS_ASSERT_MSG (IsEmpty (),
                 "SRPRoutingLSA::SRPRoutingLSA (): Non-empty LSA in constructor");
  CopyLinkRecords (lsa);
}
...
void
SRPRoutingLSA::CopyLinkRecords (const SRPRoutingLSA& lsa)
{
  NS_LOG_FUNCTION (this << &lsa);
  for (ListOfLinkRecords_t::const_iterator i = lsa.m_linkRecords.begin ();
       ...
       i++)
    {
      SRPRoutingLinkRecord *pSrc = *i;
      SRPRoutingLinkRecord *pDst = new SRPRoutingLinkRecord;
...
void
SRPRoutingLSA::ClearLinkRecords (void)
{
  NS_LOG_FUNCTION (this);
...
uint32_t
SRPRoutingLSA::AddLinkRecord (SRPRoutingLinkRecord* lr)
{
...
uint32_t
SRPRoutingLSA::GetNLinkRecords (void) const
{
  NS_LOG_FUNCTION (this);
  return m_linkRecords.size ();
}
...
SRPRoutingLSA::GetLinkRecord (uint32_t n) const
{
  ...
  NS_ASSERT_MSG (false, "SRPRoutingLSA::GetLinkRecord (): invalid index");
  return 0;
}

bool
SRPRoutingLSA::IsEmpty (void) const
{
  NS_LOG_FUNCTION (this);
  return m_linkRecords.size () == 0;
}

SRPRoutingLSA::LSType
SRPRoutingLSA::GetLSType (void) const
{
  NS_LOG_FUNCTION (this);
  return m_lsType;
}
...
void
SRPRoutingLSA::SetLSType (SRPRoutingLSA::LSType typ) 
{
...
Ipv4Address
SRPRoutingLSA::GetLinkStateId (void) const
{
  NS_LOG_FUNCTION (this);
...
void
SRPRoutingLSA::SetLinkStateId (Ipv4Address addr)
{
  NS_LOG_FUNCTION (this << addr);
  m_linkStateId = addr;
}
...
Ipv4Address
SRPRoutingLSA::GetAdvertisingRouter (void) const
{
  NS_LOG_FUNCTION (this);
...
void
SRPRoutingLSA::SetAdvertisingRouter (Ipv4Address addr)
{
  NS_LOG_FUNCTION (this << addr);
...
void
SRPRoutingLSA::SetNetworkLSANetworkMask (Ipv4Mask mask)
{
...
Ipv4Mask
SRPRoutingLSA::GetNetworkLSANetworkMask (void) const
{
  NS_LOG_FUNCTION (this);
  return m_networkLSANetworkMask;
}

SRPRoutingLSA::SPFStatus
SRPRoutingLSA::GetStatus (void) const
{
  NS_LOG_FUNCTION (this);
  return m_status;
}
...
uint32_t
SRPRoutingLSA::AddAttachedRouter (Ipv4Address addr)
{
  NS_LOG_FUNCTION (this << addr);
...
uint32_t
SRPRoutingLSA::GetNAttachedRouters (void) const
{
  NS_LOG_FUNCTION (this);
  return m_attachedRouters.size (); 
}
...
Ipv4Address
SRPRoutingLSA::GetAttachedRouter (uint32_t n) const
{
  NS_LOG_FUNCTION (this << n);
  uint32_t j = 0;
  ...
  NS_ASSERT_MSG (false, "SRPRoutingLSA::GetAttachedRouter (): invalid index");
  return Ipv4Address ("0.0.0.0");
}

void
SRPRoutingLSA::SetStatus (SRPRoutingLSA::SPFStatus status)
{
...
Ptr<Node>
SRPRoutingLSA::GetNode (void) const
{
  NS_LOG_FUNCTION (this);
...
void
SRPRoutingLSA::SetNode (Ptr<Node> node)
{
...
void
SRPRoutingLSA::Print (std::ostream &os) const
{
  NS_LOG_FUNCTION (this << &os);
  os << std::endl;
  os << "========== SRP Routing LSA ==========" << std::endl;
  os << "m_lsType = " << m_lsType;
  if (m_lsType == SRPRoutingLSA::RouterLSA) 
    {
      os << " (SRPRoutingLSA::RouterLSA)";
    }
  else if (m_lsType == SRPRoutingLSA::NetworkLSA) 
    {
      os << " (SRPRoutingLSA::NetworkLSA)";
    }
  else if (m_lsType == SRPRoutingLSA::ASExternalLSAs)
    {
      os << " (SRPRoutingLSA::ASExternalLSA)";
    }
  else
    {
      os << "(Unknown LSType)";
  ...
  if (m_lsType == SRPRoutingLSA::RouterLSA) 
    {
      for ( ListOfLinkRecords_t::const_iterator i = m_linkRecords.begin ();
            ...
            i++)
        {
          ...
          if (p->m_linkType == SRPRoutingLinkRecord::PointToPoint)
            {
              os << " (SRPRoutingLinkRecord::PointToPoint)" << std::endl;
              os << "m_linkId = " << p->m_linkId << std::endl;
              os << "m_linkData = " << p->m_linkData << std::endl;
              os << "m_metric = " << p->m_metric << std::endl;
            }
          else if (p->m_linkType == SRPRoutingLinkRecord::TransitNetwork)
            {
              os << " (SRPRoutingLinkRecord::TransitNetwork)" << std::endl;
          ...
          else if (p->m_linkType == SRPRoutingLinkRecord::StubNetwork)
            {
              os << " (SRPRoutingLinkRecord::StubNetwork)" << std::endl;
  ...
  os << "========== End SRP Routing LSA ==========" << std::endl;
}
...
//
// DiscoverLSAs is called on all nodes in the system that have a SRPRouter
...
// 
uint32_t 
SRPRouter::DiscoverLSAs ()
{
  NS_LOG_FUNCTION (this);
  Ptr<Node> node = GetObject<Node> ();
  NS_ABORT_MSG_UNLESS (node, "SRPRouter::DiscoverLSAs (): GetObject for <Node> interface failed");
  ...
  NS_ABORT_MSG_UNLESS (ipv4Local, "SRPRouter::DiscoverLSAs (): GetObject for <Ipv4> interface failed");

  //
  // Every router node originates a Router-LSA
  //
  SRPRoutingLSA *pLSA = new SRPRoutingLSA;
  pLSA->SetLSType (SRPRoutingLSA::RouterLSA);
  pLSA->SetLinkStateId (m_routerId);
  pLSA->SetAdvertisingRouter (m_routerId);
  pLSA->SetStatus (SRPRoutingLSA::LSA_SPF_NOT_EXPLORED);
  pLSA->SetNode (node);
  ...
  for (uint32_t i = 0; i < numDevices; ++i)
    {
      ...
      if (NetDeviceIsBridged (ndLocal))
        {
          ...
          NS_ABORT_MSG_IF (rc, "SRPRouter::DiscoverLSAs(): Bridge ports must not have an IPv4 interface index");
        }
      ...
      if (ndLocal->IsBroadcast () && !ndLocal->IsPointToPoint () )
        ...
        {
          NS_ASSERT_MSG (0, "SRPRouter::DiscoverLSAs (): unknown link type");
        }
    }
  ...
  for (InjectedRoutesCI i = m_injectedRoutes.begin ();
    ...
    {
      SRPRoutingLSA *pLSA = new SRPRoutingLSA;
      pLSA->SetLSType (SRPRoutingLSA::ASExternalLSAs);
...
void
SRPRouter::ProcessBroadcastLink (Ptr<NetDevice> nd, SRPRoutingLSA *pLSA, NetDeviceContainer &c)
{
...
void
SRPRouter::ProcessSingleBroadcastLink (Ptr<NetDevice> nd, SRPRoutingLSA *pLSA, NetDeviceContainer &c)
{
  NS_LOG_FUNCTION (this << nd << pLSA << &c);

  SRPRoutingLinkRecord *plr = new SRPRoutingLinkRecord;
  NS_ABORT_MSG_IF (plr == 0, "SRPRouter::ProcessSingleBroadcastLink(): Can't alloc link record");
  ...
  // there is an implied assumption that SRP routing is only going to 
  ...
  Ptr<Ipv4> ipv4Local = node->GetObject<Ipv4> ();
  NS_ABORT_MSG_UNLESS (ipv4Local, "SRPRouter::ProcessSingleBroadcastLink (): GetObject for <Ipv4> interface failed");

  // Initialize to value out of bounds to silence compiler
  uint32_t interfaceLocal = ipv4Local->GetNInterfaces () + 1;
  bool rc = FindInterfaceForDevice (node, nd, interfaceLocal);
  NS_ABORT_MSG_IF (rc == false, "SRPRouter::ProcessSingleBroadcastLink(): No interface index associated with device");
  ...
  if (AnotherRouterOnLink (nd, true) == false)
    {
      ...
      plr->SetLinkType (SRPRoutingLinkRecord::StubNetwork);
  ...
  else
    {
      ...
      plr->SetLinkType (SRPRoutingLinkRecord::TransitNetwork);
      ...
      if (desigRtr != "255.255.255.255")
        {
          ...
          NS_ABORT_MSG_UNLESS (networkHere == networkThere, 
                               "SRPRouter::ProcessSingleBroadcastLink(): Network number confusion");
        }
...
void
SRPRouter::ProcessBridgedBroadcastLink (Ptr<NetDevice> nd, SRPRoutingLSA *pLSA, NetDeviceContainer &c)
{
  NS_LOG_FUNCTION (this << nd << pLSA << &c);
  NS_ASSERT_MSG (nd->IsBridge (), "SRPRouter::ProcessBridgedBroadcastLink(): Called with non-bridge net device");
  ...
  NS_ABORT_MSG_UNLESS (bnd, "SRPRouter::DiscoverLSAs (): GetObject for <BridgeNetDevice> failed");
  ...
  Ptr<Node> node = nd->GetNode ();
  Ptr<Ipv4> ipv4Local = node->GetObject<Ipv4> ();
  NS_ABORT_MSG_UNLESS (ipv4Local, "SRPRouter::ProcessBridgedBroadcastLink (): GetObject for <Ipv4> interface failed");

  // Initialize to value out of bounds to silence compiler
  uint32_t interfaceLocal = ipv4Local->GetNInterfaces () + 1;
  bool rc = FindInterfaceForDevice (node, nd, interfaceLocal);
  NS_ABORT_MSG_IF (rc == false, "SRPRouter::ProcessBridgedBroadcastLink(): No interface index associated with device");
  ...
  SRPRoutingLinkRecord *plr = new SRPRoutingLinkRecord;
  NS_ABORT_MSG_IF (plr == 0, "SRPRouter::ProcessBridgedBroadcastLink(): Can't alloc link record");
...
void
SRPRouter::ProcessPointToPointLink (Ptr<NetDevice> ndLocal, SRPRoutingLSA *pLSA)
{
  ...
  NS_ABORT_MSG_UNLESS (ipv4Local, "SRPRouter::ProcessPointToPointLink (): GetObject for <Ipv4> interface failed");

  uint32_t interfaceLocal = ipv4Local->GetNInterfaces () + 1;
  bool rc = FindInterfaceForDevice (nodeLocal, ndLocal, interfaceLocal);
  NS_ABORT_MSG_IF (rc == false, "SRPRouter::ProcessPointToPointLink (): No interface index associated with device");
  ...
  NS_ABORT_MSG_UNLESS (ipv4Remote, 
                       "SRPRouter::ProcessPointToPointLink(): GetObject for remote <Ipv4> failed");

  //
  // Further note the requirement that nodes on either side of a point-to-point 
  // link must participate in SRP routing and therefore have a SRPRouter
  // interface aggregated.
  //
  Ptr<SRPRouter> rtrRemote = nodeRemote->GetObject<SRPRouter> ();
  if (rtrRemote == 0)
    {
      // This case is possible if the remote does not participate in SRP routing
      return;
    }
  //
  ...
  NS_ABORT_MSG_IF (rc == false, "SRPRouter::ProcessPointToPointLinks(): No interface index associated with remote device");
  ...
  //
  SRPRoutingLinkRecord *plr;
  if (ipv4Remote->IsUp (interfaceRemote))
    {
      NS_LOG_LOGIC ("Remote side interface " << interfaceRemote << " is up-- add a type 1 link");
 
      plr  = new SRPRoutingLinkRecord;
      NS_ABORT_MSG_IF (plr == 0, "SRPRouter::ProcessPointToPointLink(): Can't alloc link record");
      plr->SetLinkType (SRPRoutingLinkRecord::PointToPoint);
...
void
SRPRouter::BuildNetworkLSAs (NetDeviceContainer c)
{
  ...
  for (uint32_t i = 0; i < nDesignatedRouters; ++i)
    {
      ...
      NS_ABORT_MSG_IF (rc == false, "SRPRouter::BuildNetworkLSAs (): No interface index associated with device");
      ...
      SRPRoutingLSA *pLSA = new SRPRoutingLSA;
      NS_ABORT_MSG_IF (pLSA == 0, "SRPRouter::BuildNetworkLSAs(): Can't alloc link record");

      pLSA->SetLSType (SRPRoutingLSA::NetworkLSA);
      ...
      // and, if we find a node with a SRPRouter interface and an IPv4 
      ...
      for (uint32_t i = 0; i < nDevices; i++)
        {
          ...
          //
          // Does the node in question have a SRPRouter interface?  If not it can
          // hardly be considered an attached router.
          //
          Ptr<SRPRouter> rtr = tempNode->GetObject<SRPRouter> ();
          if (rtr == 0)
            { 
              continue;
            }
...
// attached and look for nodes with SRPRouter interfaces on them (one of them 
...
//
Ipv4Address
SRPRouter::FindDesignatedRouterForLink (Ptr<NetDevice> ndLocal, bool allowRecursion) const
{
      ...
      if (bnd)
        {
          ...
          //
          // We require a designated router to have a SRPRouter interface and
          ...
          Ptr<SRPRouter> rtr = nodeOther->GetObject<SRPRouter> ();
...
// that has a SRPRouter interface aggregated.  Life gets more complicated
// when there is a bridged net device on the other side.
//
bool
SRPRouter::AnotherRouterOnLink (Ptr<NetDevice> nd, bool allowRecursion) const
{
  ...
  for (uint32_t i = 0; i < nDevices; i++)
    {
      ...
      Ptr<SRPRouter> rtr = nodeTemp->GetObject<SRPRouter> ();
      if (rtr)
        {
          NS_LOG_LOGIC ("Found SRPRouter interface, return true");
          return true;
        }
      else 
        {
          NS_LOG_LOGIC ("No SRPRouter interface on device, continue search");
        }
    }
...
uint32_t
SRPRouter::GetNumLSAs (void) const
{
  NS_LOG_FUNCTION (this);
  return m_LSAs.size ();
}
...
//
bool
SRPRouter::GetLSA (uint32_t n, SRPRoutingLSA &lsa) const
{
  NS_LOG_FUNCTION (this << n << &lsa);
  NS_ASSERT_MSG (lsa.IsEmpty (), "SRPRouter::GetLSA (): Must pass empty LSA");
//
...
void
SRPRouter::InjectRoute (Ipv4Address network, Ipv4Mask networkMask)
{
...
SRPRouter::GetInjectedRoute (uint32_t index)
{
...
void
SRPRouter::RemoveInjectedRoute (uint32_t index)
{
  NS_LOG_FUNCTION (this << index);
...
bool
SRPRouter::WithdrawRoute (Ipv4Address network, Ipv4Mask networkMask)
{
  NS_LOG_FUNCTION (this << network << networkMask);
  for (InjectedRoutesI i = m_injectedRoutes.begin (); i != m_injectedRoutes.end (); i++)
    {
...
//
Ptr<NetDevice>
SRPRouter::GetAdjacent (Ptr<NetDevice> nd, Ptr<Channel> ch) const
{
  NS_LOG_FUNCTION (this << nd << ch);
  NS_ASSERT_MSG (ch->GetNDevices () == 2, "SRPRouter::GetAdjacent (): Channel with other than two devices");
//
...
//
  if (nd1 == nd)
    ...
    {
      NS_ASSERT_MSG (false,
                     "SRPRouter::GetAdjacent (): Wrong or confused channel?");
      return 0;
    }
}
...
//
bool
SRPRouter::FindInterfaceForDevice (Ptr<Node> node, Ptr<NetDevice> nd, uint32_t &index) const
{
...
//
Ptr<BridgeNetDevice>
SRPRouter::NetDeviceIsBridged (Ptr<NetDevice> nd) const
{
