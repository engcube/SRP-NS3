...
SPFVertex::SPFVertex (SRPRoutingLSA* lsa) : 
  ...
  m_vertexProcessed (false)
{
  ...
  if (lsa->GetLSType () == SRPRoutingLSA::RouterLSA) 
    {
      NS_LOG_LOGIC ("Setting m_vertexType to VertexRouter");
      m_vertexType = SPFVertex::VertexRouter;
    }
  else if (lsa->GetLSType () == SRPRoutingLSA::NetworkLSA) 
    { 
...
void
SPFVertex::SetLSA (SRPRoutingLSA* lsa)
{
  NS_LOG_FUNCTION (this << lsa);
  m_lsa = lsa;
}
...
// ---------------------------------------------------------------------------
//
// SRPRouteManagerLSDB Implementation
//
// ---------------------------------------------------------------------------

SRPRouteManagerLSDB::SRPRouteManagerLSDB ()
  :
    m_database (),
    m_extdatabase ()
{
...
void
SRPRouteManagerLSDB::Initialize ()
{
  NS_LOG_FUNCTION (this);
  LSDBMap_t::iterator i;
  for (i= m_database.begin (); i!= m_database.end (); i++)
    {
      SRPRoutingLSA* temp = i->second;
      temp->SetStatus (SRPRoutingLSA::LSA_SPF_NOT_EXPLORED);
    }
}

void
SRPRouteManagerLSDB::Insert (Ipv4Address addr, SRPRoutingLSA* lsa)
{
  NS_LOG_FUNCTION (this << addr << lsa);
  if (lsa->GetLSType () == SRPRoutingLSA::ASExternalLSAs) 
    {
...
SRPRoutingLSA*
SRPRouteManagerLSDB::GetExtLSA (uint32_t index) const
{
...
uint32_t
SRPRouteManagerLSDB::GetNumExtLSAs () const
{
  NS_LOG_FUNCTION (this);
...
SRPRoutingLSA*
SRPRouteManagerLSDB::GetLSA (Ipv4Address addr) const
{
...
SRPRoutingLSA*
SRPRouteManagerLSDB::GetLSAByLinkData (Ipv4Address addr) const
{
  NS_LOG_FUNCTION (this << addr);
//
// Look up an LSA by its address.
...
// Iterate among temp's Link Records
      for (uint32_t j = 0; j < temp->GetNLinkRecords (); j++)
        {
          SRPRoutingLinkRecord *lr = temp->GetLinkRecord (j);
          if ( lr->GetLinkType () == SRPRoutingLinkRecord::TransitNetwork &&
               lr->GetLinkData () == addr)
            {
              return temp;
            }
...
// ---------------------------------------------------------------------------
//
// SRPRouteManagerImpl Implementation
//
// ---------------------------------------------------------------------------

SRPRouteManagerImpl::SRPRouteManagerImpl () 
  :
    m_spfroot (0)
{
  NS_LOG_FUNCTION (this);
  m_lsdb = new SRPRouteManagerLSDB ();
}
...
void
SRPRouteManagerImpl::DebugUseLsdb (SRPRouteManagerLSDB* lsdb)
{
...
void
SRPRouteManagerImpl::DeleteSRPRoutes ()
{
  NS_LOG_FUNCTION (this);
  ...
  for (NodeList::Iterator i = NodeList::Begin (); i != listEnd; i++)
    {
      Ptr<Node> node = *i;
      Ptr<SRPRouter> router = node->GetObject<SRPRouter> ();
      if (router == 0)
        {
          continue;
        }
      Ptr<Ipv4SRPRouting> gr = router->GetRoutingProtocol ();
      uint32_t j = 0;
      ...
      for (j = 0; j < nRoutes; j++)
        {
          NS_LOG_LOGIC ("Deleting SRP route " << j << " from node " << node->GetId ());
          gr->RemoveRoute (0);
        }
      NS_LOG_LOGIC ("Deleted " << j << " SRP routes from node "<< node->GetId ());
    }
  if (m_lsdb)
    {
...
// in the system and look for those that support the SRPRouter interface.
...
//
void
SRPRouteManagerImpl::BuildSRPRoutingDatabase () 
{
  NS_LOG_FUNCTION (this);
//
// Walk the list of nodes looking for the SRPRouter Interface.  Nodes with
// SRP router interfaces are, not too surprisingly, our routers.
//
  NodeList::Iterator listEnd = NodeList::End ();
  for (NodeList::Iterator i = NodeList::Begin (); i != listEnd; i++)
    {
...
// the SRPRouter.  Afterward, you may use GetNumLSAs (), which is a very
...
// found.
//
      Ptr<Ipv4SRPRouting> grouting = rtr->GetRoutingProtocol ();
      uint32_t numLSAs = rtr->DiscoverLSAs ();
      NS_LOG_LOGIC ("Found " << numLSAs << " LSAs");

      for (uint32_t j = 0; j < numLSAs; ++j)
        {
          SRPRoutingLSA* lsa = new SRPRoutingLSA ();
//
...
//
// For each node that is a SRP router (which is determined by the presence
// of an aggregated SRPRouter interface), run the Dijkstra SPF calculation
...
//
void
SRPRouteManagerImpl::InitializeRoutes ()
{
  NS_LOG_FUNCTION (this);
//
  ...
  for (NodeList::Iterator i = NodeList::Begin (); i != listEnd; i++)
    {
      Ptr<Node> node = *i;
//
// Look for the SRPRouter interface that indicates that the node is
// participating in routing.
//
      Ptr<SRPRouter> rtr = 
        node->GetObject<SRPRouter> ();
...
//
// if the node has a SRP router interface, then run the SRP routing
// algorithms.
//
...
//
void
SRPRouteManagerImpl::SPFNext (SPFVertex* v, CandidateQueue& candidate)
{
  NS_LOG_FUNCTION (this << v << &candidate);

  SPFVertex* w = 0;
  SRPRoutingLSA* w_lsa = 0;
  SRPRoutingLinkRecord *l = 0;
  uint32_t distance = 0;
...
//
          ...
          if (l->GetLinkType () == SRPRoutingLinkRecord::StubNetwork)
            {
...
// database. 
//
          if (l->GetLinkType () == SRPRoutingLinkRecord::PointToPoint)
            {
//
...
//
              ...
              NS_LOG_LOGIC ("Found a P2P record from " << 
                   ...
                   SRPRoutingLinkRecord::TransitNetwork)
            {
              w_lsa = m_lsdb->GetLSA (l->GetLinkId ());
              NS_ASSERT (w_lsa);
...
//
      if (w_lsa->GetStatus () == SRPRoutingLSA::LSA_SPF_IN_SPFTREE) 
        {
...
//
      if (v->GetLSA ()->GetLSType () == SRPRoutingLSA::RouterLSA)
        {
...
// Is there already vertex w in candidate list?
      if (w_lsa->GetStatus () == SRPRoutingLSA::LSA_SPF_NOT_EXPLORED)
        {
...
// prepare vertex w
          ...
          if (SPFNexthopCalculation (v, w, l, distance))
            {
              w_lsa->SetStatus (SRPRoutingLSA::LSA_SPF_CANDIDATE);
//
...
//
      ...
      else if (w_lsa->GetStatus () == SRPRoutingLSA::LSA_SPF_CANDIDATE)
        {
//
...
// is very different from quagga (blame ns3::SRPRouteManagerImpl)
...
//
int
SRPRouteManagerImpl::SPFNexthopCalculation (
  SPFVertex* v, 
  SPFVertex* w,
  SRPRoutingLinkRecord* l,
  uint32_t distance)
{
...
// SRP Router Link Record contains the local IP address.  If we look at the
...
//
          NS_ASSERT (l);
          SRPRoutingLinkRecord *linkRemote = 0;
          linkRemote = SPFGetNextLink (w, v, linkRemote);
// 
// At this point, <l> is the SRP Router Link Record describing the point-
// to point link from <v> to <w> from the perspective of <v>; and <linkRemote>
// is the SRP Router Link Record describing that same link from the 
...
// W is a directly connected network; no next hop is required
          SRPRoutingLSA* w_lsa = w->GetLSA ();
          NS_ASSERT (w_lsa->GetLSType () == SRPRoutingLSA::NetworkLSA);
...
//
// Iterate through the SRP Router Link Records advertised by the vertex
...
// Setting skip to false gets us the next point-to-point SRP router link
...
//
void
SRPRouteManagerImpl::DebugSPFCalculate (Ipv4Address root)
{
...
//
bool
SRPRouteManagerImpl::CheckForStubNode (Ipv4Address root)
{
  NS_LOG_FUNCTION (this << root);
  SRPRoutingLSA *rlsa = m_lsdb->GetLSA (root);
  Ipv4Address myRouterId = rlsa->GetLinkStateId ();
  int transits = 0;
  SRPRoutingLinkRecord *transitLink = 0;
  for (uint32_t i = 0; i < rlsa->GetNLinkRecords (); i++)
    {
      SRPRoutingLinkRecord *l = rlsa->GetLinkRecord (i);
      if (l->GetLinkType () == SRPRoutingLinkRecord::TransitNetwork)
        {
          transits++;
          transitLink = l;
        }
      else if (l->GetLinkType () == SRPRoutingLinkRecord::PointToPoint)
        {
          transits++;
          transitLink = l;
        }
  ...
  if (transits == 0)
    {
      // This router is not connected to any router.  Probably, SRP
  ...
  if (transits == 1)
    {
      if (transitLink->GetLinkType () == SRPRoutingLinkRecord::TransitNetwork)
        {
      ...
      else if (transitLink->GetLinkType () == SRPRoutingLinkRecord::PointToPoint)
        {
          ...
          SRPRoutingLSA *w_lsa = m_lsdb->GetLSA (transitLink->GetLinkId ());
          ...
          for (uint32_t j = 0; j < nLinkRecords; ++j)
            {
              ...
              //
              SRPRoutingLinkRecord *lr = w_lsa->GetLinkRecord (j);
              if (lr->GetLinkType () != SRPRoutingLinkRecord::PointToPoint)
                {
                  continue;
                }
              ...
              if (lr->GetLinkId () == myRouterId)
                {
                  ...
                  Ptr<SRPRouter> router = rlsa->GetNode ()->GetObject<SRPRouter> ();
                  NS_ASSERT (router);
                  Ptr<Ipv4SRPRouting> gr = router->GetRoutingProtocol ();
                  NS_ASSERT (gr);
...
void
SRPRouteManagerImpl::SPFCalculate (Ipv4Address root)
{
  NS_LOG_FUNCTION (this << root);
...
//
  ...
  v->GetLSA ()->SetStatus (SRPRoutingLSA::LSA_SPF_IN_SPFTREE);
...
// RFC2328 16.1. (2). 
//
// We examine the SRP Router Link Records in the Link State 
...
// of the routers found in the SRP Router Link Records and added tehm to 
// the candidate list.
//
...
// point-to-point SRP Router Link Records (the links to nodes adjacent to
  ...
  for (uint32_t i = 0; i < m_lsdb->GetNumExtLSAs (); i++)
    {
      ...
      SRPRoutingLSA *extlsa = m_lsdb->GetExtLSA (i);
...
void
SRPRouteManagerImpl::ProcessASExternals (SPFVertex* v, SRPRoutingLSA* extlsa)
{
...
void
SRPRouteManagerImpl::SPFAddASExternal (SRPRoutingLSA *extlsa, SPFVertex *v)
{
  NS_LOG_FUNCTION (this << extlsa << v);

  NS_ASSERT_MSG (m_spfroot, "SRPRouteManagerImpl::SPFAddASExternal (): Root pointer not set");
  ...
  for (; i != listEnd; i++)
    {
      Ptr<Node> node = *i;
//
// The router ID is accessible through the SRPRouter interface, so we need
// to QI for that interface.  If there's no SRPRouter interface, the node
...
// 
      ...
      if (rtr == 0)
        {
          NS_LOG_LOGIC ("No SRPRouter interface on node " << node->GetId ());
          continue;
        }
//
...
//
          ...
          NS_ASSERT_MSG (ipv4, 
                         "SRPRouteManagerImpl::SPFIntraAddRouter (): "
                         "QI for <Ipv4> interface failed");
//
// Get the SRP Router Link State Advertisement from the vertex we're
// adding the routes to.  The LSA will have a number of attached SRP Router
...
void
SRPRouteManagerImpl::SPFProcessStubs (SPFVertex* v)
{
  NS_LOG_FUNCTION (this << v);
      ...
      for (uint32_t i = 0; i < rlsa->GetNLinkRecords (); i++)
        {
          NS_LOG_LOGIC ("Examining link " << i << " of " << 
                        v->GetVertexId () << "'s " <<
                        v->GetLSA ()->GetNLinkRecords () << " link records");
          SRPRoutingLinkRecord *l = v->GetLSA ()->GetLinkRecord (i);
          if (l->GetLinkType () == SRPRoutingLinkRecord::StubNetwork)
            {
              NS_LOG_LOGIC ("Found a Stub record to " << l->GetLinkId ());
              SPFIntraAddStub (l, v);
...
void
SRPRouteManagerImpl::SPFIntraAddStub (SRPRoutingLinkRecord *l, SPFVertex* v)
{
  NS_LOG_FUNCTION (this << l << v);

  NS_ASSERT_MSG (m_spfroot, 
                 "SRPRouteManagerImpl::SPFIntraAddStub (): Root pointer not set");
...
// 
      ...
      if (rtr == 0)
        {
          NS_LOG_LOGIC ("No SRPRouter interface on node " << 
                        node->GetId ());
          continue;
        }
//
...
//
int32_t
SRPRouteManagerImpl::FindOutgoingInterfaceId (Ipv4Address a, Ipv4Mask amask)
{
  ...
  for (; i != listEnd; i++)
    {
      ...
      Ptr<SRPRouter> rtr = 
        node->GetObject<SRPRouter> ();
//
// If the node doesn't have a SRPRouter interface it can't be the one
// we're interested in.
//
      if (rtr == 0)
        {
...
//
          Ptr<Ipv4> ipv4 = node->GetObject<Ipv4> ();
          NS_ASSERT_MSG (ipv4, 
                         "SRPRouteManagerImpl::FindOutgoingInterfaceId (): "
...
//
          ...
          if (interface < 0)
            {
              NS_FATAL_ERROR ("SRPRouteManagerImpl::FindOutgoingInterfaceId(): "
...
//
void
SRPRouteManagerImpl::SPFIntraAddRouter (SPFVertex* v)
{
  NS_LOG_FUNCTION (this << v);

  NS_ASSERT_MSG (m_spfroot, 
                 "SRPRouteManagerImpl::SPFIntraAddRouter (): Root pointer not set");
//
// The root of the Shortest Path First tree is the router to which we are 
// going to write the actual routing table entries.  The vertex corresponding
// to this router has a vertex ID which is the router ID of that node.  We're
...
//
// The router ID is accessible through the SRPRouter interface, so we need
// to GetObject for that interface.  If there's no SRPRouter interface, 
...
// We are only concerned about point-to-point links
//
              SRPRoutingLinkRecord *lr = lsa->GetLinkRecord (j);
              if (lr->GetLinkType () != SRPRoutingLinkRecord::PointToPoint)
                {
                  continue;
                }
...
void
SRPRouteManagerImpl::SPFIntraAddTransit (SPFVertex* v)
{
  NS_LOG_FUNCTION (this << v);

  NS_ASSERT_MSG (m_spfroot, 
                 "SRPRouteManagerImpl::SPFIntraAddTransit (): Root pointer not set");
//
// The root of the Shortest Path First tree is the router to which we are 
// going to write the actual routing table entries.  The vertex corresponding
// to this router has a vertex ID which is the router ID of that node.  We're
...
//
          Ptr<Ipv4> ipv4 = node->GetObject<Ipv4> ();
          NS_ASSERT_MSG (ipv4, 
                         "SRPRouteManagerImpl::SPFIntraAddTransit (): "
                         "GetObject for <Ipv4> interface failed");
//
// Get the SRP Router Link State Advertisement from the vertex we're
// adding the routes to.  The LSA will have a number of attached SRP Router
...
//
void
SRPRouteManagerImpl::SPFVertexAddParent (SPFVertex* v)
{
  NS_LOG_FUNCTION (this << v);
