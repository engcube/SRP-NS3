...
void downAction(){
    ...
    ConfLoader::Instance()->getNodeContainer().Get (downNode1)->GetObject<Ipv4SRPRouting>()->removeNeighbor(downNode2);
    ConfLoader::Instance()->getNodeContainer().Get (downNode2)->GetObject<Ipv4SRPRouting>()->removeNeighbor(downNode1);
}
...
void upAction(){
    ...
    ConfLoader::Instance()->getNodeContainer().Get (downNode1)->GetObject<Ipv4SRPRouting>()->addNeighbor(downNode2);
    ConfLoader::Instance()->getNodeContainer().Get (downNode2)->GetObject<Ipv4SRPRouting>()->addNeighbor(downNode1);
}

void init(){
    for(int i=0; i<ConfLoader::Instance()->getTotalNum()+ConfLoader::Instance()->getToRNum(); i++){
        ConfLoader::Instance()->getNodeContainer().Get(i)->GetObject<Ipv4SRPRouting>()->initGrid();
    }
}
...
void Hello(){
  cout << Simulator::Now() << "----------------hello---------"<<endl;
    for(int i=0; i<ConfLoader::Instance()->getTotalNum()+ConfLoader::Instance()->getToRNum(); i++){
        ConfLoader::Instance()->getNodeContainer().Get(i)->GetObject<Ipv4SRPRouting>()->sendHelloMessage();
    }
}
...
void CheckNeighbor(){
  cout << Simulator::Now() << "----------------checkNeighbor---------"<<endl;
    for(int i=0; i<ConfLoader::Instance()->getTotalNum()+ConfLoader::Instance()->getToRNum(); i++){
        ConfLoader::Instance()->getNodeContainer().Get(i)->GetObject<Ipv4SRPRouting>()->checkNeighbors();
    }
}
...
int main (int argc, char *argv[])
{
  ...
  uint32_t packetReceiveDelay = 1000;
  int maxPackets = 1000;
  ...
  createApplication(nNodes+0, nNodes+1, port, sendRate, packetSize, app_start_time, app_stop_time);
